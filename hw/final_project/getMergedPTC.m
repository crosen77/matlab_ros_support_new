function [ptCloud_pic, nonPlane_pic, ptCloud_world, base_to_cam_pose, cam_to_base_pose] = getMergedPTC(zoneInspect, ang_offset, optns) 
%--------------------------------------------------------------------------    
...
%--------------------------------------------------------------------------
    ...
    if strcmp(zoneInspect, 'Zone3')
        displace_gripper(mat_R_T_G,optns,'u', -0.5, 0, 0);
    end
    ...
    for iter = 1:length(locations)
        ...
        if strcmp(zoneInspect, 'Zone3') & iter == 4
            displaceG = displace_gripper(cur_gripper_location,optns,locations{iter},0.25);
        % These motions rotate the gripper inwards at end of displacement
        % to center object.
        elseif iter < 5
            displaceG = displace_gripper(cur_gripper_location,optns,locations{iter},0.07);
        else
            displaceGA = displace_gripper(cur_gripper_location,optns,locations{iter},0.07,1,ang_offset);
        end
    ...
    if strcmp(zoneInspect, 'Zone4')
        moveToQ('custom', optns, [-0.1453 0.4926 0.8193 -1.3119 0 -0.1453]);
    else    
